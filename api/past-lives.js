  ...
  "lives": [
    {
      ...
      "identity": "직업명 (상세설명) 형식 (예: '구미호 (서촌 떡집 심부름꾼 위장)', '기생 (한양 최고 명기)', '무속인 (남원 떠돌이 무당)', '무신 (고려 무신정권 장수)')",
      "name": "지역명 + 이름 형식 (예: '서촌 연이', '남원 박신녀', '개경 최도원', '한양 월매')",
      ...
      "group": "아래 12개 중 반드시 하나: fantasy(구미호·도깨비·이무기·저승사자 등 초자연) / warrior(무인·장수·전사·협객) / shaman(무당·무속인·신관·점쟁이) / entertainer(기생·악사·광대·예인) / commoner(노비·농부·어부·평민) / scholar(선비·의원·학자·관리·암행어사) / royal(왕·왕비·왕녀·황제·황후) / noble(양반·귀부인·사대부·귀족) / monk(승려·수도자·도사) / court(궁녀·내시·시녀·환관) / outlaw(도적·반란군·해적·자객) / outcast(백정·광인·천민·사형수)",
      "gender": "남 또는 여",
...
위 정보로 정확히 ${totalLives}개의 전생을 JSON으로 생성하세요. 시드값 ${hash}을 기반으로 항상 동일한 결과를 반환하세요.`;
  ...
  try {
    ...
    if (start === -1 || end === -1 || end < start) {
      console.error('[parse] no JSON braces found in content:', text.slice(0, 500));
