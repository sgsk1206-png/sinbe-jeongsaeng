...
// lifeIndex별 group 결정 — hash + lifeIndex 기반으로 12개 고르게 순환
// 7은 12와 서로소 → lifeIndex 1~12 모두 서로 다른 그룹 배정 보장
...
function assignGroup(hash, lifeIndex) {
  const base = parseInt(hash.slice(0, 4), 16); // 사람마다 다른 기준점
...
━━ 필드 형식 규칙 (반드시 준수) ━━
- identity: 반드시 "직업명 (상세한 역할 설명)" 형식. 지정된 group에 어울리는 직업을 창의적으로 생성할 것.
  예) "백정 (한양 도성 가죽 장인)", "해녀 (제주 앞바다 전복 채취인)", "야장 (비밀 결사 무기 제조인)"
...
- past_trace: 화면에서 "전생의 {직업명} 때문에 이번생엔..." 접두어가 자동으로 붙으므로,
  여기서는 그 이후의 결과/특성 부분만 한 문장으로 작성할 것. 중복 절대 금지.
  예) "고통 속에서도 남을 먼저 챙기는 습관이 있다" / "물을 보면 이유 없이 눈물이 난다"
  "전생의..." 문구를 직접 포함하면 중복되므로 절대 쓰지 말 것.
...
- death: 최후 묘사는 아래 5가지 유형 중 하나를 시드값에 따라 선택. 비극으로만 치우치지 말 것.
  · 자연사/천수 — 평화롭게, 조용히 눈을 감음
  · 쓸쓸한 마무리 — 홀로, 타향에서, 잊혀진 채로 생을 마침
  · 황당한 죽음 — 뜻밖의 사고, 어이없는 최후
  · 비장한 죽음 — 전투, 희생, 대의를 위한 결말
  · 억울한 죽음 — 누명, 모함, 부당한 죽음
- story: 태어난 배경, 살아온 삶, 주요 사건, 전환점을 담은 입체적인 서사. 최소 3문장 이상 작성.

━━ 오행 분석으로 전생 인물 성격 결정 (생년월일 기반) ━━
group은 별도로 지정됩니다. 오행은 인물의 기질·이야기 방향에만 반영하세요.
- 목(木) 강함 → 도전적·개혁적 기질. 변화를 주도하고 새 길을 여는 인물.
- 화(火) 강함 → 열정적·리더십 기질. 불꽃처럼 치열하게 살다 간 인물.
- 토(土) 강함 → 안정적·수호적 기질. 묵묵히 자리를 지키며 살아온 인물.
- 금(金) 강함 → 냉철·원칙적 기질. 명예와 의리를 끝까지 좇은 인물.
- 수(水) 강함 → 신비·예술적 기질. 물처럼 변화무쌍하고 감수성 깊은 인물.
- 도화살 → 타인을 끌어당기는 강렬한 매력의 소유자.
- 역마살 → 한곳에 정착하지 못하고 끊임없이 떠돌던 삶.
- 귀문관살 → 초자연적 감각과 기이한 체험을 겪은 인물.
- 백호살 → 극적이고 강렬한 최후를 맞이한 인물.
- 공망 → 이루려 했던 꿈이 번번이 허사로 돌아간 인물.
...
━━ 반환 형식 ━━
{
  "life": {
    ...
    "death": "최후를 한 문장으로 — 자연사/쓸쓸한마무리/황당한죽음/비장한죽음/억울한죽음 중 하나",
    "karma": "이 전생이 현생에 남긴 업보를 한 문장으로",
    "past_trace": "이번 생에 나타나는 특성을 한 문장으로. '전생의...' 문구 포함 금지 (UI에서 자동 추가됨)",
...
export default async function handler(req, res) {
  ...
  // hash + lifeIndex 기반으로 12개 그룹 고르게 배정 (결정론적)
  const assignedGroup = assignGroup(hash, lifeIndex);

  const usedFiguresList = Array.isArray(usedFigures) && usedFigures.length > 0
    ? `\n이미 사용된 역사 인물 (절대 중복 금지): ${usedFigures.join(', ')}`
    : '';

  // 사주팔자·오행·신살 계산 (그룹 결정에 사용하지 않음 — 인물 성격/이야기 방향에만 활용)
  let sajuSection = '';
  try {
    ...
    sajuSection = `
이 사람의 사주 분석 (인물 성격·이야기 방향에 반영할 것 — group 결정과 무관):
...
- 신살: ${sinsal.length > 0 ? sinsal.join(', ') : '없음'}${traits.length > 0 ? `\n- 신살 특성: ${traits.join(' / ')}` : ''}`;
    console.log(`[manse] dominant=${dominant} sinsal=${sinsal.join(',')} assignedGroup=${assignedGroup}`);
  } catch (e) {
...
영혼 등급: ${soulGrade}
이 전생의 group은 반드시 "${assignedGroup}"이어야 합니다. (시드 기반 확정값, 절대 변경 불가)${usedFiguresList}${sajuSection}
